...
public class ExpressionDocumenter {
	...
	public static Map<String, Expression> load() throws ExpressionDocumentationException {
		if (expressionDocumentation == null) {
			...
			for (String root : EXPRESSION_ROOTS) {
				try (InputStream docStream = docLoader
						.getResourceAsStream(EXPRESSION_DOCO_ROOTPATH + root + ".paramdoc.xml")) {
					if (docStream != null) {
	 ...
	 * <origin><![CDATA[ <distributionManagement> <repository> <id>some-repo</id>
	 * <url>scp://host/path</url> </repository> <snapshotRepository>
	 * <id>some-snap-repo</id> <url>scp://host/snapshot-path</url>
	 * </snapshotRepository> </distributionManagement> ]]></origin> <usage><![CDATA[
	 * The repositories onto which artifacts should be deployed. One is for
	 * releases, the other for snapshots. ]]></usage> </expression> <expressions>
	 *
	 * @throws IOException
