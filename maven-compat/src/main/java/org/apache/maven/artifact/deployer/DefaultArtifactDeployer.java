...
public class DefaultArtifactDeployer extends AbstractLogEnabled implements ArtifactDeployer {
	...
	/**
	 * @deprecated we want to use the artifact method only, and ensure artifact.file
	 *             is set correctly.
	 */
	@Deprecated
	...
	public void deploy(File source, Artifact artifact, ArtifactRepository deploymentRepository,
			ArtifactRepository localRepository) throws ArtifactDeploymentException {
		RepositorySystemSession session = LegacyLocalRepositoryManager.overlay(localRepository,
		...
		/*
		 * NOTE: This provides backward-compat with maven-deploy-plugin:2.4 which
		 * bypasses the repository factory when using an alternative deployment
		 * location.
		 */
