...
public class DefaultRepositoryMetadataManager extends AbstractLogEnabled implements RepositoryMetadataManager {
	...
	public void resolve(RepositoryMetadata metadata, RepositoryRequest request)
		...
		if (!request.isOffline()) {
			...
			for (ArtifactRepository repository : remoteRepositories) {
				...
				File file = new File(localRepo.getBasedir(),
						localRepo.pathOfLocalRepositoryMetadata(metadata, repository));
				boolean update;
		...
		try {
			...
			throw new RepositoryMetadataResolutionException("Unable to store local copy of metadata: " + e.getMessage(),
					e);
		}
	}
	...
	private void mergeMetadata(RepositoryMetadata metadata, List<ArtifactRepository> remoteRepositories,
			ArtifactRepository localRepository) throws RepositoryMetadataStoreException {
		// TODO currently this is first wins, but really we should take the latest by
		// comparing either the
		// snapshot timestamp, or some other timestamp later encoded into the metadata.
		// TODO this needs to be repeated here so the merging doesn't interfere with the
		// written metadata
		// - we'd be much better having a pristine input, and an ongoing metadata for
		// merging instead
	...
	private void updateSnapshotMetadata(RepositoryMetadata metadata, Map<ArtifactRepository, Metadata> previousMetadata,
			ArtifactRepository selected, ArtifactRepository localRepository) throws RepositoryMetadataStoreException {
		// TODO this could be a lot nicer... should really be in the snapshot
		// transformation?
	...
	/**
	 * Ensures the last updated timestamp of the specified metadata does not refer
	 * to the future and fixes the local metadata if necessary to allow proper
	 * merging/updating of metadata during deployment.
	 */
	...
	private File getArtifactMetadataFromDeploymentRepository(ArtifactMetadata metadata, ArtifactRepository localRepo,
			ArtifactRepository remoteRepository) throws TransferFailedException {
		File file = new File(localRepo.getBasedir(),
				localRepo.pathOfLocalRepositoryMetadata(metadata, remoteRepository));
