...
public class DefaultMavenProjectBuilder implements MavenProjectBuilder {
	...
	private ProjectBuildingRequest toRequest(ProjectBuilderConfiguration configuration) {
		...
		if (profileManager != null) {
			 ...
			 * impossible for plugins to access the global profile manager which is required
			 * to build a POM like a CLI invocation does. Failure to consider the activated
			 * profiles can cause repo declarations to be lost which in turn will result in
			 * artifact resolution failures, in particular when using the enhanced local
			 * repo which guards access to local files based on the configured remote repos.
			 */
	...
	private List<ArtifactRepository> normalizeToArtifactRepositories(List<?> repositories,
		...
		/*
		 * This provides backward-compat with 2.x that allowed plugins like the
		 * maven-remote-resources-plugin:1.0 to populate the builder configuration with
		 * model repositories instead of artifact repositories.
		 */
	...
	public MavenProject buildFromRepository(Artifact artifact, List<ArtifactRepository> remoteRepositories,
			ProjectBuilderConfiguration configuration, boolean allowStubModel) throws ProjectBuildingException {
		ProjectBuildingRequest request = injectSession(toRequest(configuration));
	...
	/**
	 * This is used for pom-less execution like running archetype:generate. I am
