 ...
 * POM.
 *
 * @author jdcasey Created on Feb 3, 2005 TODO Consolidate this logic with the
 *         PluginParameterExpressionEvaluator, minus deprecations/bans.
 */
@Deprecated
public abstract class AbstractStringBasedModelInterpolator extends AbstractLogEnabled
	...
	static {
		...
		// If the build section is present and the project directory is non-null, we
		// should make
		// sure interpolation of the directories below uses translated paths.
		// Afterward, we'll double back and translate any paths that weren't covered
		// during interpolation via the
		// code below...
	...
	/**
	 * Serialize the inbound Model instance to a StringWriter, perform the regex
	 * replacement to resolve POM expressions, then re-parse into the resolved Model
	 * instance.
	 * <p>
	 ...
	 *
	 * @deprecated Use
	 *             {@link ModelInterpolator#interpolate(Model, File, ProjectBuilderConfiguration, boolean)}
	 *             instead.
	 */
	 ...
	 * <li>If the value is null, but the context contains the expression, don't
	 * replace the expression string with the value, and continue to find other
	 * expressions.</li>
	 * <li>If the value is null, get it from the model properties.</li>
	 * </ul>
	 */
	public String interpolate(String src, Model model, final File projectDir, ProjectBuilderConfiguration config,
			boolean debug) throws ModelInterpolationException {
		try {
	...
	protected String interpolateInternal(String src, List<ValueSource> valueSources,
			List<InterpolationPostProcessor> postProcessors, boolean debug) throws ModelInterpolationException {
		if (!src.contains("${")) {
			return src;
		}
