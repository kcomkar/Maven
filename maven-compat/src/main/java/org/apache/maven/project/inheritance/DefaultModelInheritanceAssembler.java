...
public class DefaultModelInheritanceAssembler implements ModelInheritanceAssembler {
	...
	private void assembleModelInheritance(Model child, Model parent, String childPathAdjustment, boolean appendPaths) {
		...
		if (child.getVersion() == null) {
			// The parent version may have resolved to something different, so we take what
			// we asked for...
		...
		if (parent.getDistributionManagement() != null) {
			...
			if (childDistMgmt.getSnapshotRepository() == null) {
				if (parentDistMgmt.getSnapshotRepository() != null) {
					DeploymentRepository repository = copyDistributionRepository(
							parentDistMgmt.getSnapshotRepository());
			...
			// NOTE: We SHOULD NOT be inheriting status, since this is an assessment of the
			// POM quality.
			// NOTE: We SHOULD NOT be inheriting relocation, since this relates to a single
			// POM
		}
	}
