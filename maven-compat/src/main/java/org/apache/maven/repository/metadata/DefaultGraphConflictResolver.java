...
public class DefaultGraphConflictResolver implements GraphConflictResolver {
			...
			for (MetadataGraphVertex v : vertices) {
				...
				if (edge == null) { // no edges - don't need this vertex any more
					if (entry.equals(v)) { // unless it's an entry point.
											// currently processing the entry point - it should not have any entry
											// incident edges
					...
					// System.out.println("+++>"+v.getMd().toDomainString()+" still has
					// "+edge.toString() );
					// fill in domain md with actual version data
					ArtifactMetadata md = v.getMd();
					ArtifactMetadata newMd = new ArtifactMetadata(md.getGroupId(), md.getArtifactId(),
							edge.getVersion(), md.getType(), md.getScopeAsEnum(), md.getClassifier(),
							edge.getArtifactUri(), edge.getSource() == null ? "" : edge.getSource().getMd().toString(),
							edge.isResolved(), edge.getTarget() == null ? null : edge.getTarget().getMd().getError());
	...
	private void visit(MetadataGraphVertex from, List<MetadataGraphVertex> visited, MetadataGraph graph) {
		...
		// String s = "|---> "+from.getMd().toString()+" - "+(exitList == null ? -1 :
		// exitList.size()) + " exit links";
