...
public class MetadataGraph {
	 ...
	 * @param versionedVertices true if graph nodes should be versioned (different
	 *                          versions -&gt; different nodes)
	 * @param scopedVertices    true if graph nodes should be versioned and scoped
	 *                          (different versions and/or scopes -&gt; different
	 *                          nodes)
	 *
	 */
	...
	private void processTreeNodes(MetadataGraphVertex parentVertex, MetadataTreeNode node, int depth, int pomOrder)
		...
		if (parentVertex != null) // then create the edge
		{
			...
			MetadataGraphEdge e = new MetadataGraphEdge(md.version, md.resolved, md.artifactScope, md.artifactUri,
					depth, pomOrder);
