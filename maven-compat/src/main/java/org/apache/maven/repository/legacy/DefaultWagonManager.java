...
/**
 * Manages <a href="https://maven.apache.org/wagon">Wagon</a> related operations
 * in Maven.
 */
...
public class DefaultWagonManager implements WagonManager {
	...
	public void getArtifact(Artifact artifact, ArtifactRepository repository, TransferListener downloadMonitor,
			boolean force) throws TransferFailedException, ResourceDoesNotExistException {
	...
	public void getArtifact(Artifact artifact, List<ArtifactRepository> remoteRepositories,
		...
		// if it already exists locally we were just trying to force it - ignore the
		// update
	...
	public void getArtifactMetadataFromDeploymentRepository(ArtifactMetadata metadata, ArtifactRepository repository,
			File destination, String checksumPolicy) throws TransferFailedException, ResourceDoesNotExistException {
		String remotePath = repository.pathOfRemoteRepositoryMetadata(metadata);
	...
	public void getRemoteFile(ArtifactRepository repository, File destination, String remotePath,
		...
		try {
			...
			// this will run at most twice. The first time, the firstRun flag is turned off,
			// and if the retry flag
			// is set on the first run, it will be turned off and not re-set on the second
			// try. This is because the
			...
			while (firstRun || retry) {
				...
				if (downloaded) {
					...
					try {
						...
						// if we catch a ChecksumFailedException, it means the transfer/read succeeded,
						// but the
						// checksum doesn't match. This could be a problem with the server (ibiblio
						// HTTP-200 error
						// page), so we'll try this up to two times. On the second try, we'll handle it
						// as a bona-fide
						...
						// if this IS NOT a ChecksumFailedException, it was a problem with transfer/read
						// of the checksum
						// file...we'll try again with the MD5 checksum.
						try {
							verifyChecksum(md5ChecksumObserver, destination, temp, remotePath, ".md5", wagon);
						} catch (ChecksumFailedException e) {
							// if we also fail to verify based on the MD5 checksum, and the checksum
							// transfer/read
							// succeeded, then we need to determine whether to retry or handle it as a
							// failure.
							if (firstRun) {
								retry = true;
							} else {
			...
			// Remove remaining TransferListener instances (checksum handlers removed in
			// above finally clause)
			if (downloadMonitor != null) {
				wagon.removeTransferListener(downloadMonitor);
			}
		...
		if (downloaded) {
			...
			// The temporary file is named destination + ".tmp" and is done this way to
			// ensure
			// that the temporary file is in the same file system as the destination because
			// the
			// File.renameTo operation doesn't really work across file systems.
			// So we will attempt to do a File.renameTo for efficiency and atomicity, if
			// this fails
	...
	public void putArtifact(File source, Artifact artifact, ArtifactRepository deploymentRepository,
			TransferListener downloadMonitor) throws TransferFailedException {
	...
	public void putRemoteFile(ArtifactRepository repository, File source, String remotePath,
		...
		try {
			...
			// We do this in here so we can checksum the artifact metadata too, otherwise it
			// could be metadata itself
			for (String extension : checksums.keySet()) {
