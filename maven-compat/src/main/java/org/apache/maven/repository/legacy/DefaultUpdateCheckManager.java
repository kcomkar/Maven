...
public class DefaultUpdateCheckManager extends AbstractLogEnabled implements UpdateCheckManager {
	...
	public boolean isUpdateRequired(RepositoryMetadata metadata, ArtifactRepository repository, File file) {
		// Here, we need to determine which policy to use. Release updateInterval will
		// be used when
		// the metadata refers to a release artifact or meta-version, and snapshot
		// updateInterval will be used when
		// it refers to a snapshot artifact or meta-version.
		// NOTE: Release metadata includes version information about artifacts that have
		// been released, to allow
		// meta-versions like RELEASE and LATEST to resolve, and also to allow retrieval
		// of the range of valid, released
		// artifacts available.
	...
	String getRepositoryKey(ArtifactRepository repository) {
		...
		// consider the username&password because a repo manager might block artifacts
		...
		// consider the URL (instead of the id) as this most closely relates to the
