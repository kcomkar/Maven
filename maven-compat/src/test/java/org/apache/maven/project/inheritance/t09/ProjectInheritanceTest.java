...
/**
 * Verifies exclusions listed in dependencyManagement are valid for transitive
 * dependencies.
 *
...
public class ProjectInheritanceTest extends AbstractProjectInheritanceTestCase {
	 ...
	 *
	 * 1. dependencyManagement lists dependencies on a &amp; b, with an exclusion on
	 * c in b. 2. the child project lists a dependency on project a only 3. a
	 * depends on b (which is transitive to the child project), and b depends on c.
	 *
	 * We should see that the resulting size of collected artifacts is two: a &amp;
	 * b only.
	 */
	...
	/**
	 * Setup exactly the same as the above test, except that the child project now
	 * depends upon d, which has a transitive dependency on c. Even though we did
	 * list an exclusion on c, it was only from within the context of project b. We
	 * will pick up project c in this case because no restrictions were placed on d.
	 * This demonstrates that a, b, c, &amp; d will all be collected.
	 *
	 * @throws Exception
	 */
