...
public class ProjectClasspathTest extends AbstractMavenProjectTestCase {
	...
	public void setUp() throws Exception {
		...
		DefaultArtifactDescriptorReader pomReader = (DefaultArtifactDescriptorReader) lookup(
	...
	public void testProjectClasspath() throws Exception {
		...
		// check all transitive deps of a test dependency are test, except test and
		// provided which is skipped
		...
		// check all transitive deps of a provided dependency are provided scope, except
		// for test
		...
		// check all transitive deps of a runtime dependency are runtime scope, except
		// for test
		...
		// check all transitive deps of a compile dependency are compile scope, except
		// for runtime and test
		checkGroupIdScope(project, "compile", "maven-test-compile");
		artifact = getArtifact(project, "maven-test-compile", "scope-runtime");
		assertEquals("Check scope", "runtime", artifact.getScope());

		// check all transitive deps of a default dependency are compile scope, except
		// for runtime and test
