...
public abstract class AbstractCoreMavenComponentTestCase extends PlexusTestCase {
	...
	/**
	 * We need to customize the standard Plexus container with the plugin discovery
	 * listener which is what looks for the META-INF/maven/plugin.xml resources that
	 * enter the system when a Maven plugin is loaded.
	 *
	 * We also need to customize the Plexus container with a standard plugin
	 * discovery listener which is the MavenPluginCollector. When a Maven plugin is
	 * discovered the MavenPluginCollector collects the plugin descriptors which are
	 * found.
	 */
	...
	protected MavenExecutionRequest createMavenExecutionRequest(File pom) throws Exception {
		MavenExecutionRequest request = new DefaultMavenExecutionRequest().setPom(pom).setProjectPresent(true)
				.setShowErrors(true).setPluginGroups(Arrays.asList("org.apache.maven.plugins"))
	...
	// layer the creation of a project builder configuration with a request, but
	// this will need to be
	// a Maven subclass because we don't want to couple maven to the project builder
	// which we need to
	// separate.
