...
public class PomConstructionTest extends PlexusTestCase {
	...
	// Some better conventions for the test poms needs to be created and each of
	// these tests
	// that represent a verification of a specification item needs to be a couple
	// lines at most.
	// The expressions help a lot, but we need a clean to pick up a directory of
	// POMs, automatically load
	// them into a resolver, create the expression to extract the data to validate
	// the Model, and the URI
	// to validate the properties. We also need a way to navigate from the Tex
	// specification documents to
	// the test in question and vice versa. A little Eclipse plugin would do the
	// trick.
	...
	/*
	 * MNG- public void testDependencyScope() throws Exception { PomTestWrapper pom
	...
	/**
	 * MNG-4005: postponed to 3.1 public void
	 * testValidationErrorUponNonUniqueDependencyKey() throws Exception { try {
	 * buildPom( "unique-dependency-key/deps" ); fail( "Non-unique dependency keys
	 * did not cause validation error" ); } catch ( ProjectBuildingException e ) {
	 * // expected } }
	 * 
	 * public void testValidationErrorUponNonUniqueDependencyManagementKey() throws
	 * Exception { try { buildPom( "unique-dependency-key/dep-mgmt" ); fail(
	 * "Non-unique dependency keys did not cause validation error" ); } catch (
	 * ProjectBuildingException e ) { // expected } }
	 * 
	 * public void testValidationErrorUponNonUniqueDependencyKeyInProfile() throws
	 * Exception { try { buildPom( "unique-dependency-key/deps-in-profile" ); fail(
	 * "Non-unique dependency keys did not cause validation error" ); } catch (
	 * ProjectBuildingException e ) { // expected } }
	 * 
	 * public void
	 * testValidationErrorUponNonUniqueDependencyManagementKeyInProfile() throws
	 * Exception { try { buildPom( "unique-dependency-key/dep-mgmt-in-profile" );
	 * fail( "Non-unique dependency keys did not cause validation error" ); } catch
	 * ( ProjectBuildingException e ) { // expected } }
	 */
	...
	/*
	 * public void testMaven() throws Exception { PomTestWrapper pom =
	 * buildPomFromMavenProject( "maven-build/sub/pom.xml", null );
	 * 
	...
	public void testReportingInterpolation() throws Exception {
		...
		assertEquals(createPath(Arrays.asList(System.getProperty("user.dir"), "src", "test",
				"resources-project-builder", "reporting-interpolation", "target", "site")),
	...
	/*
	 * public void testPluginConfigDuplicate() throws Exception { PomTestWrapper pom
	 * = buildPom( "plugin-config-duplicate/dup" ); }
	 */
	...
	public void testSingleConfigurationInheritance() throws Exception {
		...
		assertEquals("2.0.6", pom
				.getValue("build/plugins[1]/executions[1]/configuration[1]/rules[1]/requireMavenVersion[1]/version"));
		assertEquals("[1.4,)",
	...
	public void testDifferentContainersWithSameId() throws Exception {
		...
		assertEquals(1, ((List<?>) pom
				.getValue("build/pluginManagement/plugins[@artifactId='maven-it-plugin-b']/executions[1]/goals"))
						.size());
	}
	...
	public void testXmlTextCoalescing() throws Exception {
		...
		assertEquals(32 * 1024, pom.getValue("properties/prop2").toString().trim().replaceAll("[\n\r]", "").length());
	}
	...
	public void testInterpolationWithBasedirAlignedDirectories() throws Exception {
		...
		assertEquals(new File(pom.getBasedir(), "target"), new File(pom.getValue("properties/buildOut").toString()));
	...
	public void testDefaultPluginsExecutionContributedByPackagingExecuteBeforeUserDefinedExecutions() throws Exception {
		...
		@SuppressWarnings("unchecked")
		List<PluginExecution> executions = (List<PluginExecution>) pom
				.getValue("build/plugins[@artifactId='maven-resources-plugin']/executions");
