...
public class LifecycleExecutorTest extends AbstractCoreMavenComponentTestCase {
	...
	protected void setUp() throws Exception {
		...
		lifeCycleTaskSegmentCalculator = (DefaultLifecycleTaskSegmentCalculator) lookup(
	...
	// -----------------------------------------------------------------------------------------------
	// Tests which exercise the lifecycle executor when it is dealing with default
	// lifecycle phases.
	// -----------------------------------------------------------------------------------------------

	public void testCalculationOfBuildPlanWithIndividualTaskWherePluginIsSpecifiedInThePom() throws Exception {
		// We are doing something like "mvn resources:resources" where no version is
		// specified but this
		// project we are working on has the version specified in the POM so the version
	...
	public void testCalculationOfBuildPlanWithIndividualTaskOfTheCleanLifecycle() throws Exception {
		// We are doing something like "mvn clean:clean" where no version is specified
		// but this
		// project we are working on has the version specified in the POM so the version
		// should come from there.
		File pom = getProject("project-basic");
	...
	public void testSetupMojoExecution() throws Exception {
		...
		MavenExecutionPlan executionPlan = lifeCycleExecutionPlanCalculator.calculateExecutionPlan(session,
				session.getCurrentProject(), Arrays.asList((Object) task), false);
