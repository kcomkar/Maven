...
public class DefaultLifecyclesTest extends PlexusTestCase {
	...
	public void testCustomLifecycle() {
		...
		Lifecycle myLifecycle = new Lifecycle("etl", Arrays.asList("extract", "transform", "load"),
		...
		DefaultLifecycles dl = new DefaultLifecycles(
				myLifecycles.stream().collect(Collectors.toMap(l -> l.getId(), l -> l)), null);
	...
	private Lifecycle getLifeCycleById(String id) {
		return defaultLifeCycles.getLifeCycles().stream().filter(l -> id.equals(l.getId())).findFirst()
