...
public class PluginManagerTest extends AbstractCoreMavenComponentTestCase {
	...
	// TODO These two tests display a lack of symmetry with respect to the input
	// which is a free form string and the
	// mojo descriptor which comes back. All the free form parsing needs to be done
	// somewhere else, this is
	// really the function of the CLI, and then the pre-processing of that output
	// still needs to be fed into
	// a hinting process which helps flesh out the full specification of the plugin.
	// The plugin manager should
	// only deal in concrete terms -- all version finding mumbo jumbo is a
	// customization to base functionality
	...
	public void testRemoteResourcesPlugin() throws Exception {
		...
		/*
		 * 
		 * This will not work until the RR plugin is released to get rid of the binding
		 * to the reporting exception which is a mistake.
		 * 
		 ...
		 * "project-with-inheritance" ) ); String goal = "process";
		 * 
		 * Plugin plugin = new Plugin(); plugin.setGroupId( "org.apache.maven.plugins"
		 * ); plugin.setArtifactId( "maven-remote-resources-plugin" );
		 * plugin.setVersion( "1.0-beta-2" );
		 * 
		 * MojoDescriptor mojoDescriptor = pluginManager.getMojoDescriptor( plugin,
		 * goal, session.getCurrentProject(), session.getLocalRepository() );
		 * assertPluginDescriptor( mojoDescriptor, "org.apache.maven.plugins",
		 * "maven-remote-resources-plugin", "1.0-beta-2" ); MojoExecution mojoExecution
		 * = new MojoExecution( mojoDescriptor ); pluginManager.executeMojo( session,
		 * mojoExecution );
		 */
	}
	...
	public void testSurefirePlugin() throws Exception {
		 ...
		 * 
		 * Plugin plugin = new Plugin(); plugin.setGroupId( "org.apache.maven.plugins"
		 * ); plugin.setArtifactId( "maven-surefire-plugin" ); plugin.setVersion(
		 * "2.4.2" );
		 * 
		 ...
		 * MojoDescriptor mojoDescriptor = pluginManager.getMojoDescriptor( plugin,
		 * goal, session.getLocalRepository(),
		 ...
		 * ).getExecutions().get( 0 ).getConfiguration(); MojoExecution mojoExecution =
		 * new MojoExecution( mojoDescriptor, configuration );
	 ...
	 * underlying tool. Common case is in the Antlr plugin which comes bundled with
	 * a version of Antlr but the user often times needs to use a specific version.
	...
	/**
	 * The case where you have a plugin in the current build that you want to be
	...
	/**
	 * This is the case where the Mojo wants to execute on every project and then do
	...
	/**
	 * This is the case where a Mojo needs the lifecycle run to a certain phase
	...
	public void testThatPluginDependencyThatHasSystemScopeIsResolved() throws Exception {
		...
		for (Artifact a : artifacts) {
			if (a.getGroupId().equals("org.apache.maven.its.mng3586") && a.getArtifactId().equals("tools")) {
				// The system scoped dependencies will be present in the classloader for the
				// plugin
				return;
			}
		}
