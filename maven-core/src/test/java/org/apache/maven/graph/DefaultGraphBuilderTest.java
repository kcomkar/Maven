...
public class DefaultGraphBuilderTest {
	...
	public static Collection<Object[]> parameters() {
		return asList(scenario("Full reactor").expectResult(asList(INDEPENDENT_MODULE, MODULE_A, MODULE_B, MODULE_C)),
				scenario("Selected project").selectedProjects(singletonList(MODULE_B))
						.expectResult(singletonList(MODULE_B)),
				scenario("Excluded project").excludedProjects(singletonList(MODULE_B))
						.expectResult(asList(INDEPENDENT_MODULE, MODULE_A, MODULE_C)),
				scenario("Resuming from project").resumeFrom(MODULE_B).expectResult(asList(MODULE_B, MODULE_C)),
				scenario("Selected project with also make dependencies").selectedProjects(singletonList(MODULE_C))
						.makeBehavior(REACTOR_MAKE_UPSTREAM).expectResult(asList(MODULE_A, MODULE_B, MODULE_C)),
				scenario("Selected project with also make dependents").selectedProjects(singletonList(MODULE_B))
						.makeBehavior(REACTOR_MAKE_DOWNSTREAM).expectResult(asList(MODULE_B, MODULE_C)),
				scenario("Resuming from project with also make dependencies").makeBehavior(REACTOR_MAKE_UPSTREAM)
						.resumeFrom(MODULE_C).expectResult(asList(MODULE_A, MODULE_B, MODULE_C)),
				scenario("Selected project with resume from an also make dependency (MNG-4960 IT#1)")
						.selectedProjects(singletonList(MODULE_C)).resumeFrom(MODULE_B)
						.makeBehavior(REACTOR_MAKE_UPSTREAM).expectResult(asList(MODULE_A, MODULE_B, MODULE_C)),
				scenario("Selected project with resume from an also make dependent (MNG-4960 IT#2)")
						.selectedProjects(singletonList(MODULE_B)).resumeFrom(MODULE_C)
						.makeBehavior(REACTOR_MAKE_DOWNSTREAM).expectResult(singletonList(MODULE_C)),
				scenario("Excluding an also make dependency from selectedProject does take its transitive dependency")
						.selectedProjects(singletonList(MODULE_C)).excludedProjects(singletonList(MODULE_B))
						.makeBehavior(REACTOR_MAKE_UPSTREAM).expectResult(asList(MODULE_A, MODULE_C)),
				scenario("Excluding an also make dependency from resumeFrom does take its transitive dependency")
						.resumeFrom(MODULE_C).excludedProjects(singletonList(MODULE_B))
						.makeBehavior(REACTOR_MAKE_UPSTREAM).expectResult(asList(MODULE_A, MODULE_C)),
				scenario("Resume from exclude project downstream").resumeFrom(MODULE_A)
						.excludedProjects(singletonList(MODULE_B)).expectResult(asList(MODULE_A, MODULE_C)),
				scenario("Exclude the project we are resuming from (as proposed in MNG-6676)").resumeFrom(MODULE_B)
						.excludedProjects(singletonList(MODULE_B)).expectResult(singletonList(MODULE_C)),
				scenario("Selected projects in wrong order are resumed correctly in order")
						.selectedProjects(asList(MODULE_C, MODULE_B, MODULE_A)).resumeFrom(MODULE_B)
						.expectResult(asList(MODULE_B, MODULE_C)),
				scenario("Duplicate projects are filtered out").selectedProjects(asList(MODULE_A, MODULE_A))
						.expectResult(singletonList(MODULE_A)));
	}

	public DefaultGraphBuilderTest(String description, List<String> selectedProjects, List<String> excludedProjects,
			String resumedFrom, String makeBehavior, List<String> expectedReactorProjects) {
	...
	public void testGetReactorProjects() {
		// Given
		List<String> selectedProjects = parameterSelectedProjects.stream().map(p -> ":" + p)
				.collect(Collectors.toList());
		List<String> excludedProjects = parameterExcludedProjects.stream().map(p -> ":" + p)
				.collect(Collectors.toList());
		...
		List<MavenProject> expectedReactorProjects = parameterExpectedResult.stream().map(artifactIdProjectMap::get)
				.collect(Collectors.toList());
	...
	public void before() throws Exception {
		...
		when(projectBuilder.build(anyList(), anyBoolean(), any(ProjectBuildingRequest.class))).thenReturn(
				asList(projectBuildingResult1, projectBuildingResult2, projectBuildingResult3, projectBuildingResult4));

		artifactIdProjectMap = ImmutableMap.of(INDEPENDENT_MODULE, projectIndependentModule, MODULE_A, projectModuleA,
				MODULE_B, projectModuleB, MODULE_C, projectModuleC);
	}
	...
	static class ScenarioBuilder {
		...
		public Object[] expectResult(List<String> expectedReactorProjects) {
			return new Object[] { description, selectedProjects, excludedProjects, resumeFrom, makeBehavior,
					expectedReactorProjects };
		}
	}
}