...
public class DefaultMavenPluginManager implements MavenPluginManager {
	...
	/**
	 * <p>
	 * PluginId =&gt; ExtensionRealmCache.CacheRecord map MavenProject context value
	 * key. The map is used to ensure the same class realm is used to load build
	 * extensions and load mojos for extensions=true plugins.
	 * </p>
	 * <strong>Note:</strong> This is part of internal implementation and may be
	...
	public synchronized void setupPluginRealm(PluginDescriptor pluginDescriptor, MavenSession session,
		...
		if (plugin.isExtensions()) {
			...
			if (cacheRecord != null) {
				...
				cacheRecord = pluginRealmCache.put(cacheKey, pluginDescriptor.getClassRealm(),
	...
	private void createPluginRealm(PluginDescriptor pluginDescriptor, MavenSession session, ClassLoader parent,
		...
		DependencyNode root = pluginDependenciesResolver.resolve(plugin, RepositoryUtils.toArtifact(pluginArtifact),
				dependencyFilter, project.getRemotePluginRepositories(), repositorySession);
	...
	private void discoverPluginComponents(final ClassRealm pluginRealm, Plugin plugin,
		...
		try {
			...
			throw new PluginContainerException(plugin, pluginRealm,
					"Error in component graph of plugin " + plugin.getId() + ": " + e.getMessage(), e);
		}
	}
	...
	public <T> T getConfiguredMojo(Class<T> mojoInterface, MavenSession session, MojoExecution mojoExecution)
		...
		// We are forcing the use of the plugin realm for all lookups that might occur
		// during
		// the lifecycle that is part of the lookup. Here we are specifically trying to
		// keep
			...
			try {
				...
				throw new PluginContainerException(mojoDescriptor, pluginRealm,
						"Unable to load the mojo '" + mojoDescriptor.getGoal()
								+ "' (or one of its required components) from the plugin '" + pluginDescriptor.getId()
								+ "'",
						e);
			}
	...
	private void populatePluginFields(Object mojo, MojoDescriptor mojoDescriptor, ClassRealm pluginRealm,
		...
		try {
			// TODO could the configuration be passed to lookup and the configurator known
			// to plexus via the descriptor
			...
			if (!missingParameters.isEmpty()) {
				if ("basic".equals(configuratorId)) {
					...
					/*
					 * NOTE: Other configurators like the map-oriented one don't call into the
					 * listener, so do it the hard way.
					 */
	...
	private void validateParameters(MojoDescriptor mojoDescriptor, PlexusConfiguration configuration,
			ExpressionEvaluator expressionEvaluator) throws ComponentConfigurationException, PluginParameterException {
