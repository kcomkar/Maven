 ...
 * <code>${</code> and <code>}</code> is evaluated. Recognized values are:
 * <table border="1">
...
public class PluginParameterExpressionEvaluator implements TypeAwareExpressionEvaluator {
	...
	public PluginParameterExpressionEvaluator(MavenSession session, MojoExecution mojoExecution) {
		...
		//
		// Maven4: We may want to evaluate how this is used but we add these separate as
		// the
	...
	public Object evaluate(String expr, Class<?> type) throws ExpressionEvaluationException {
		...
		/*
		 * MNG-4312: We neither have reserved all of the above magic expressions nor is
		 * their set fixed/well-known (it gets occasionally extended by newer Maven
		 * versions). This imposes the risk for existing plugins to unintentionally use
		 * such a magic expression for an ordinary system property. So here we check
		 * whether we ended up with a magic value that is not compatible with the type
		 * of the configured mojo parameter (a string could still be converted by the
		 * configurator so we leave those alone). If so, back off to evaluating the
	...
	private static boolean isTypeCompatible(Class<?> type, Object value) {
		...
		// likely Boolean -> boolean, Short -> int etc. conversions, it's not the
		// problem case we try to avoid
	...
	public File alignToBaseDirectory(File file) {
		// TODO Copied from the DefaultInterpolator. We likely want to resurrect the
		// PathTranslator or at least a
		// similar component for re-usage
		if (file != null) {
			if (file.isAbsolute()) {
				// path was already absolute, just normalize file separator and we're done
			} else if (file.getPath().startsWith(File.separator)) {
				// drive-relative Windows path, don't align with project directory but with
				// drive root
