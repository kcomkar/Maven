...
/**
 * <p>
 * Presents a view of the Dependency Graph that is suited for concurrent
 * building.
 * </p>
...
public class ConcurrencyDependencyGraph {
	 ...
	 * @return The list of builds that are eligible for starting now that the
	 *         provided project is done
	 */
	...
	private List<MavenProject> getSchedulableNewProcesses(MavenProject finishedProject) {
		...
		for (MavenProject dependentProject : projectDependencyGraph.getDownstreamProjects(finishedProject, false)) {
			final List<MavenProject> upstreamProjects = projectDependencyGraph.getUpstreamProjects(dependentProject,
					false);
	...
	/**
	 * For the given {@link MavenProject} {@code p}, return all of {@code p}'s
	 * dependencies.
	 *
	 * @param p
