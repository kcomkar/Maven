...
public class RepositoryUtils {
	...
	public static org.apache.maven.artifact.Artifact toArtifact(Artifact artifact) {
		...
		/*
		 * NOTE: From Artifact.hasClassifier(), an empty string and a null both denote
		 * "no classifier". However, some plugins only check for null, so be sure to
		 * nullify an empty classifier.
		 */
	...
	public static Artifact toArtifact(org.apache.maven.artifact.Artifact artifact) {
		...
		Artifact result = new DefaultArtifact(artifact.getGroupId(), artifact.getArtifactId(), artifact.getClassifier(),
	...
	public static String getLayout(ArtifactRepository repo) {
		try {
			...
			/*
			 * NOTE: getId() was added in 3.x and is as such not implemented by plugins
			 * compiled against 2.x APIs.
			 */
