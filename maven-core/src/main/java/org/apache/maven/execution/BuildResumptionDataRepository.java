 ...
 * feature. This data is used to ensure newer builds of the same project, that
 * have the -r command-line flag, skip successfully built projects during
 * earlier invocations of Maven.
 */
public interface BuildResumptionDataRepository {
	/**
	 * Persists any data needed to resume the build at a later point in time, using
	 * a new Maven invocation. This method may also decide it is not needed or
	 * meaningful to persist such data, and return <code>false</code> to indicate
	 * so.
	 *
