...
public class DefaultBuildResumptionAnalyzer implements BuildResumptionAnalyzer {
	...
	public Optional<BuildResumptionData> determineBuildResumptionData(final MavenExecutionResult result) {
		...
		if (isFailedProjectFirstInBuild(result, resumeFromProject)) {
			// As the first module in the build failed, there is no need to specify this as
			// the resumeFrom project.
	...
	private List<MavenProject> getFailedProjectsInOrder(MavenExecutionResult result) {
		...
		return result.getExceptions().stream().filter(LifecycleExecutionException.class::isInstance)
				.map(LifecycleExecutionException.class::cast).map(LifecycleExecutionException::getProject)
				.filter(Objects::nonNull).sorted(comparing(sortedProjects::indexOf)).collect(Collectors.toList());
	}

	/**
	 * Projects after the first failed project could have succeeded by using -T or
	 * --fail-at-end. These projects can be skipped from later builds. This is not
	 * the case these projects are dependent on one of the failed projects.
	 * 
	 * @param result                The result of the Maven build.
	 * @param failedProjects        The list of failed projects in the build.
	 * @param startFromProjectIndex Start looking for projects which can be skipped
	...
	private List<String> determineProjectsToSkip(MavenExecutionResult result, List<MavenProject> failedProjects,
		...
		List<GroupArtifactPair> failedProjectsGAList = failedProjects.stream().map(GroupArtifactPair::new)
				.collect(Collectors.toList());

		return remainingProjects.stream().filter(project -> result.getBuildSummary(project) instanceof BuildSuccess)
				.filter(project -> hasNoDependencyOnProjects(project, failedProjectsGAList))
				.map(project -> project.getGroupId() + ":" + project.getArtifactId()).collect(Collectors.toList());
	}

	private boolean hasNoDependencyOnProjects(MavenProject project, List<GroupArtifactPair> projectsGAs) {
		return project.getDependencies().stream().map(GroupArtifactPair::new).noneMatch(projectsGAs::contains);
	}
