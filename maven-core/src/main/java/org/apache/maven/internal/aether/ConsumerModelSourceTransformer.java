...
class ConsumerModelSourceTransformer extends AbstractModelSourceTransformer {
	...
	protected TransformerHandler getTransformerHandler(Path pomFile)
		...
		final SAXTransformerFactory transformerFactory = (SAXTransformerFactory) Factories.newTransformerFactory();
		...
		try (InputStream input = Files.newInputStream(pomFile)) {
			...
			throw new org.apache.maven.model.building.TransformerException("Failed to detect XML encoding and version",
					e);
		}
