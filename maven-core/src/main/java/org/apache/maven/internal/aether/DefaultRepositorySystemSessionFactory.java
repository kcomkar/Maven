...
public class DefaultRepositorySystemSessionFactory {
	...
	public DefaultRepositorySystemSession newRepositorySession(MavenExecutionRequest request) {
		...
		for (Proxy proxy : decrypted.getProxies()) {
			...
			proxySelector.add(new org.eclipse.aether.repository.Proxy(proxy.getProtocol(), proxy.getHost(),
					proxy.getPort(), authBuilder.build()), proxy.getNonProxyHosts());
		}
	...
	private Collection<FileTransformer> getTransformersForArtifact(final Artifact artifact,
		...
		// In case of install:install-file there's no transformer context, as the goal
		// is unrelated to the lifecycle.
