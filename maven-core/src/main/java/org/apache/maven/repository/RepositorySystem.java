...
public interface RepositorySystem {
	...
	/**
	 * Calculates the effective repositories for the given input repositories which
	 * are assumed to be already mirrored (if applicable). This process will
	 * essentially remove duplicate repositories by merging them into one equivalent
	 * repository. It is worth to point out that merging does not simply choose one
	 * of the input repositories and discards the others but actually combines their
	 * possibly different policies.
	 *
	 * @param repositories The original repositories, may be {@code null}.
	 * @return The effective repositories or {@code null} if the input was
	 *         {@code null}.
	 */
	 ...
	 *
	 * @param repository The repository to determine the mirror for, must not be
	 *                   {@code null}.
	 * @param mirrors    The available mirrors, may be {@code null}.
	 * @return The mirror specification for the repository or {@code null} if no
	 *         mirror matched.
	 */
	 ...
	 * repository that is matched by a mirror, its URL and ID will be updated to
	 * match the values from the mirror specification. Repositories without a
	 * matching mirror will pass through unchanged. <em>Note:</em> This method must
	 * be called before {@link #injectAuthentication(List, List)} or the
	 ...
	 * repository that is matched by a proxy, its proxy data will be set
	 * accordingly. Repositories without a matching proxy will have their proxy
	 * cleared. <em>Note:</em> This method must be called after
	 * {@link #injectMirror(List, List)} or the repositories will end up with the
	 * wrong proxies.
	 *
	 * @param repositories The repositories into which to inject the proxy
	 *                     information, may be {@code null}.
	...
	/**
	 * Injects the authentication information into the specified repositories. For
	 * each repository that is matched by a server, its credentials will be updated
	 * to match the values from the server specification. Repositories without a
	 * matching server will have their credentials cleared. <em>Note:</em> This
	 * method must be called after {@link #injectMirror(List, List)} or the
	 * repositories will end up with the wrong credentials.
	 *
	 * @param repositories The repositories into which to inject the authentication
	 *                     information, may be {@code null}.
