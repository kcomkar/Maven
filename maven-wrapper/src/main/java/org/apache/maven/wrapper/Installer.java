...
public class Installer {
	...
	public Path createDist(WrapperConfiguration configuration) throws IOException, URISyntaxException {
		URI distributionUrl;
	...
	private List<Path> listDirs(Path distDir) throws IOException {
		return Files.walk(distDir, 1).filter(p -> !distDir.equals(p)).filter(Files::isDirectory)
