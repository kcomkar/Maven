...
public class DefaultSettingsBuilder implements SettingsBuilder {
	...
	public SettingsBuildingResult build(SettingsBuildingRequest request) throws SettingsBuildingException {
		...
		Source globalSettingsSource = getSettingsSource(request.getGlobalSettingsFile(),
				request.getGlobalSettingsSource());
		...
		// for the special case of a drive-relative Windows path, make sure it's
		// absolute to save plugins from trouble
	...
	private Settings readSettings(Source settingsSource, SettingsBuildingRequest request,
		...
		try {
			...
			problems.add(SettingsProblem.Severity.FATAL,
					"Non-parseable settings " + settingsSource.getLocation() + ": " + e.getMessage(), e.getLineNumber(),
					e.getColumnNumber(), e);
			return new Settings();
		} catch (IOException e) {
			problems.add(SettingsProblem.Severity.FATAL,
