...
public class DefaultVersionResolver implements VersionResolver, Service {
	...
	public VersionResult resolveVersion(RepositorySystemSession session, VersionRequest request)
		...
		if (metadata == null) {
			...
			for (RemoteRepository repository : request.getRepositories()) {
				MetadataRequest metadataRequest = new MetadataRequest(metadata, repository,
				...
				try (SyncContext syncContext = syncContextFactory.newInstance(session, true)) {
					...
					if (metadata.getFile() != null && metadata.getFile().exists()) {
						try (InputStream in = new FileInputStream(metadata.getFile())) {
							...
							/*
							 * NOTE: Users occasionally misuse the id "local" for remote repos which screws
							 * up the metadata of the local repository. This is especially troublesome
							 * during snapshot resolution so we try to handle that gracefully.
							 */
	...
	private boolean isSafelyCacheable(RepositorySystemSession session, Artifact artifact) {
		 ...
		 * information for any of its artifacts/projects.
		 */
	...
	private static class Key {
		...
		public boolean equals(Object obj) {
			...
			return artifactId.equals(that.artifactId) && groupId.equals(that.groupId)
					...
					&& version.equals(that.version) && context.equals(that.context) && localRepo.equals(that.localRepo)
