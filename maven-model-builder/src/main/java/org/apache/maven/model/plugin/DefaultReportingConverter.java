...
/**
 * Handles conversion of the <code>&lt;reporting&gt;</code> section into the
 * configuration of Maven Site Plugin 3.x, i.e. <code>reportPlugins</code> and
 * <code>outputDirectory</code> parameters.
 *
...
public class DefaultReportingConverter implements ReportingConverter {
	...
	public void convertReporting(Model model, ModelBuildingRequest request, ModelProblemCollector problems) {
		...
		if (reportPlugins != null) {
			// new-style report configuration already present: warn since this new style has
			// been deprecated
		...
		/*
		 * waiting for MSITE-484 before deprecating <reporting> section if (
		 * !reporting.getPlugins().isEmpty() && request.getValidationLevel() >=
		 ...
		 * + " section of the new Maven Site Plugin." ) .setLocation(
		 * reporting.getLocation( "" ) ) ); }
		 */
