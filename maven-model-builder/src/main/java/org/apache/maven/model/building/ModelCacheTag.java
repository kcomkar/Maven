...
/**
 * Describes a tag used by the model builder to access a {@link ModelCache}.
 * This interface basically aggregates a name and a class to provide some type
 * safety when working with the otherwise untyped cache.
 *
...
interface ModelCacheTag<T> {
	...
	/**
	 * Creates a copy of the data suitable for storage in the cache. The original
	 * data to store can be mutated after the cache is populated but the state of
	 * the cache must not change so we need to make a copy.
	 *
	 ...
	 * retrieved data can be mutated after the cache is queried but the state of the
	 * cache must not change so we need to make a copy.
	 *
	...
	/**
	 * The tag used to denote an effective dependency management section from an
	 * imported model.
	 */
