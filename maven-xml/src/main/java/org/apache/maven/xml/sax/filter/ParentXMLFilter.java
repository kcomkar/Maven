...
/**
 * <p>
 * Transforms relativePath to version. We could decide to simply allow
 * {@code <parent/>}, but let's require the GA for now for checking This filter
 * does NOT remove the relativePath (which is done by
 * {@link RelativePathXMLFilter}, it will only optionally include the version
 * based on the path
 * </p>
 * 
 ...
 * @since 3.7.0
 */
class ParentXMLFilter extends AbstractEventXMLFilter {
	...
	@Override
	public void startElement(String uri, final String localName, String qName, Attributes atts) throws SAXException {
			...
			case "parent":
				...
				if (!hasVersion && resolvedParent.isPresent()) {
					try (Includer i = super.include()) {
						super.characters(parentWhitespace.toCharArray(), 0, parentWhitespace.length());
						...
						super.characters(resolvedParentVersion.toCharArray(), 0, resolvedParentVersion.length());
	...
	protected Optional<RelativeProject> resolveRelativePath(Path relativePath) {
		Optional<RelativeProject> mappedProject = relativePathMapper
				.apply(projectPath.resolve(relativePath).normalize());
