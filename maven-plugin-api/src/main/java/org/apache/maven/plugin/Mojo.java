...
/**
 * This interface forms the contract required for <code>Mojos</code> to interact
 * with the <code>Maven</code> infrastructure.<br>
 * It features an <code>execute()</code> method, which triggers the Mojo's
 * build-process behavior, and can throw a MojoExecutionException or
 * MojoFailureException if error conditions occur.<br>
 * Also included is the <code>setLog(...)</code> method, which simply allows
 * Maven to inject a logging mechanism which will allow the Mojo to communicate
 * to the outside world through standard Maven channels.
 *
...
public interface Mojo {
	 ...
	 * <code>Mojo</code> to communicate events and feedback to the user.
	 *
	...
	/**
	 * Furnish access to the standard Maven logging mechanism which is managed in
	 * this base class.
	 *
	 * @return a log4j-like logger object which allows plugins to create messages at
	 *         levels of <code>"debug"</code>, <code>"info"</code>,
	 *         <code>"warn"</code>, and <code>"error"</code>. This logger is the
	 *         accepted means to display information to the user.
	 */
	Log getLog();
}
