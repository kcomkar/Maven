...
public class PluginDescriptor extends ComponentSetDescriptor implements Cloneable {
	...
	public void addMojo(MojoDescriptor mojoDescriptor) throws DuplicateMojoDescriptorException {
		...
		if (existing != null) {
			throw new DuplicateMojoDescriptorException(getGoalPrefix(), mojoDescriptor.getGoal(),
					existing.getImplementation(), mojoDescriptor.getImplementation());
		} else {
	...
	/**
	 * Gets the artifacts that make up the plugin's class realm, excluding artifacts
	 * shadowed by the Maven core realm like {@code maven-project}.
	 *
	...
	public MojoDescriptor getMojo(String goal) {
		...
		// TODO could we use a map? Maybe if the parent did that for components too, as
