...
public class ComparableVersion implements Comparable<ComparableVersion> {
	...
	private static class StringItem implements Item {
		 ...
		 *
		 * just returning an Integer with the index here is faster, but requires a lot
		 * of if/then/else to check for -1 or QUALIFIERS.size and then resort to lexical
		 * ordering. Most comparisons are decided by the first character, so this is
		 * still fast. If more characters are needed then it requires a lexical sort
		 * anyway.
		 *
		 * @param qualifier
	...
	/**
	 * Represents a version list item. This class is used both for the global item
	 * list and for sub-lists (which start with '-(number)' in the version
	 * specification).
	 */
	private static class ListItem extends ArrayList<Item> implements Item {
		...
		/**
		 * Return the contents in the same format that is used when you call toString()
		 * on a List.
		 */
	 ...
	 * </pre>
	 *
	 * @param args the version strings to parse and compare. You can pass arbitrary
	 *             number of version strings and always two adjacent will be
	 *             compared
	 */
